...
const TalentCard = (props: any) => {
    ...
    const [profile, setTProfile] = useState<any>(null);
    const user = useSelector((state: any) => state.user);
    const token = useSelector((state: any) => state.jwt);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [chatOpened, setChatOpened] = useState(false); // State to control chat modal
    useEffect(() => {
        if (token) {
    ...
    const handleMessageClick = () => {
        setChatOpened(true); // Open chat modal
      };
    return <div data-aos="fade-up" className="p-4 rounded-xl bg-mine-shaft-900   hover:shadow-[0_0_5px_1px_yellow] !shadow-bright-sun-400  transition duration-300 ease-in-out w-96 bs-mx:w-[48%] md-mx:w-full flex flex-col gap-3">
        ...
        <div className="flex [&>*]:w-1/2 [&>*]:p-1">
            {
                !props.invited && <>
                    ...
                    <div>
                        {props.posted ? <Button color="brightSun.4" variant="light" onClick={open} rightSection={<IconCalendarMonth className="w-5 h-5" />} fullWidth>Schedule</Button> : <Button color="brightSun.4" variant="light" onClick={handleMessageClick} fullWidth>Message</Button>}
                    </div>
                </>
            }{
      ...
      onChange={(event) => setTime(event.currentTarget.value)}  minTime="" onClick={() => ref.current?.showPicker()} />
        ...
        <Modal opened={chatOpened} onClose={() => setChatOpened(false)} radius="lg" title="Chat" centered size="lg">
        <ChatComponent senderId={user?.email} receiverId={profile?.email} />
      </Modal>
    </div>
}
